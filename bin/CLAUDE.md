...
- `bin/load-agent-ssh-keys`: Load every `~/.ssh/agents/*/id_ed25519` into the ssh-agent, skipping ones already there. Run at login by the `com.technicalpickles.agent-ssh-keys` LaunchAgent so *launchd's* agent gets populated, not just the fish shell's -- without it, scheduled jobs can't sign commits (see [LaunchAgents/README.md](../LaunchAgents/README.md)).
