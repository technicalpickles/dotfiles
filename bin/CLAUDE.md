...
- `bin/safari-reading-list`: Exports Safari's Reading List by parsing `~/Library/Safari/Bookmarks.plist` directly (stdlib `plistlib`, no deps). Human-readable title/url list by default; `--json` for full JSON (title, url, preview, date_added, unread); `--unread-only` filters to unread items; `--debug` dumps one raw item's plist keys for troubleshooting. Requires Full Disk Access for the terminal app running it -- macOS TCC blocks reading Safari's data otherwise.
